...
##
# Build dblocks-core from local source

(android_build_verify_so "libdblocks_core.so" "libczmq.so" &> /dev/null) || {
    rm -rf "${cache}/dblocks-core"
    (cp -r ../.. "${cache}/dblocks-core" && cd "${cache}/dblocks-core" \
        && make clean && rm -f configure config.status)

    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'

    (cd "${cache}/dblocks-core" && ./autogen.sh \
...
android_build_verify_so "libdblocks_core.so" "libczmq.so"
