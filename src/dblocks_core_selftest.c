/*  =========================================================================
    dblocks_core_selftest.c - run selftests
...
static test_item_t
all_tests [] = {
    { "myclass", myclass_test },
    { "someother", someother_test },
    { "myactor", myactor_test },
    { "someactor", someactor_test },
    {0, 0}          //  Sentinel
};
...
{
    test_item_t *item;
    printf ("Running dblocks-core selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("dblocks_core_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running dblocks-core test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
